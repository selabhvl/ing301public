...
def read_file(file_name):
    text = open(str(file_name),mode = "r",encoding="utf-8")
...
def lines_to_words(lines):
    ...
    for x in lines:
        words += x.lower().split()
    for x in words:
        stripped_words.append(x.strip(". , : ; ? !"))
        #print(stripped_words)
    while "" in stripped_words:
        stripped_words.remove("")
...
def remove_filler_words(frequency_table):
    counter = len(frequency_table)
    keys = list(frequency_table)

    for i in range(counter):
        for word in FILL_WORDS:
            if word == keys[i]:
                del frequency_table[keys[i]]
...
def find_most_frequent(frequency_table):
    ...
    for i in range(counter):
        if int(frequency_table.get(keys[i],None)) == int(max_val):
            svar = keys[i]
