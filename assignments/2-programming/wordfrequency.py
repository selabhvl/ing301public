...
def lines_to_words(lines):
    all_words = []
    for line in lines:
        no_punct = line.translate(line.maketrans("", "", string.punctuation))
        lower = no_punct.lower()
        words = lower.split()
        all_words.extend(words)
    return all_words

# all_words=lines_to_words(lines)
# print(all_words)
...
# frequency_table = compute_frequency(words)
...
def remove_filler_words(frequency_table, Fill_WORDS):
    for word in Fill_WORDS:
        if word in frequency_table:
            del frequency_table[word]
    return frequency_table

# frequency_table = remove_filler_words(frequency_table, FILL_WORDS)
# print(frequency_table)
...
def largest_pair(par_1, par_2):
    if par_1[1] > par_2[1]:
        ...
        return 'Enten er det noe er feil, eller så er den ene ikke større enn den andre'

# largest_pair = largest_pair(par_1,par_2)
# print(largest_pair)
...
# most_frequent_word = find_most_frequent(frequency_table)
...
def main():
    ...
    # print(table)
    table = remove_filler_words(table, FILL_WORDS)
    # print(table)
