...
def lines_to_words(lines):
    ...
    processed_words = []
    for line in lines:
        words = re.findall(r'\b\w+\b', line) #Here I import a re-library to make a list of not overlapping words 
        for word in words:
            word = word.lower()
            word = word.strip(".,:;,?,!")
...
def remove_filler_words(frequency_table):
    ...
    for fill_word in FILL_WORDS:
        if fill_word in frequency_table:
            del frequency_table[fill_word]
    return frequency_table
